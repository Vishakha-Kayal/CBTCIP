...
const Dashboard = () => {
  ...
  const editBtn = async () => {
    const response = await axios.post(`${url}/users/editThis/${userId}`)
  }
  const deleteBtn = async () => {
    const response = await axios.post(`${url}/users/deleteThis/${userId}`)
  }
  ...
  return (
    <div className='w-full min-h-screen'>
      ...
      <div className='px-12 py-9 w-full flex'>
        <div
          className="w-[50%] eventDiv flex flex-col mb-2 cursor-pointer hover:bg-[#f7f6f5] transition-all ease-linear 2s h-[27rem]"
        >
